...
namespace Devinno.Forms.Controls
{
    public class DvColorPicker : DvControl
    {
        ...
        #region CodeType
        private ColorCodeType eCodeType = ColorCodeType.CodeRGB;
        public ColorCodeType CodeType
        {
            get => eCodeType;
            set
            {
        ...
        protected override void OnThemeDraw(PaintEventArgs e, DvTheme Theme)
        {
            ...
            Areas((rtContent, rtTitle, rtValue, rtButton, rtValueAll) =>
            {
                ...
                #region Value
                {
                    ...
                    Util.TextIconBounds(g, rtValue, DvContentAlignment.MiddleCenter,
                       s, Font, 3, new SizeF(sz, sz), DvTextIconAlignment.LeftRight,
                       (rtIcon, rtText) =>
                       {
                           rtIcon.Offset(0, 1);
                           Theme.DrawBox(e.Graphics, rtIcon, SelectedColor, Color.Black, RoundType.Rect, Box.FlatBox(true, true));
                           Theme.DrawText(e.Graphics, s, Font, ForeColor, rtText);
                       });
                ...
                if (useB)
                {
                    ...
                    Theme.DrawIcon(g, new DvIcon("fa-palette", 12), cT, rtButton);
                }
                #endregion
            });
            ...
            Areas((rtContent, rtTitle, rtValue, rtButton, rtValueAll) =>
            {
                ...
                if (ButtonDownState)
                {
                    ...
                    if (ButtonWidth.HasValue && CollisionTool.Check(rtButton, e.Location))
                    {
                        ...
                        var c = dlg.ShowColorPicker(Title ?? "색상 선택", SelectedColor);
                        if (c.HasValue) SelectedColor = c.Value;
