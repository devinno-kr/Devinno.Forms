...
namespace Devinno.Forms.Containers
{
    public class DvSplitterTableLayoutPanel : DvTableLayoutPanel
    {
        ...
        private Color? cSplitterColor = null;
        ...
        List<DvSplitter> Splitter = new List<DvSplitter>();
        Timer tmr = new Timer();
        Timer tmr2 = new Timer();
        Size PrevSize = Size.Empty;

        DvSplitter downSplitter = null;
        ...
        {
            #region Color
            var SplitterColor = this.SplitterColor ?? Theme.WindowTitleColor;
            #endregion
            #region Set
            ...
            if (cols.Length == ColumnStyles.Count && rows.Length == RowStyles.Count)
            {
                ...
                for (int i = 0; i < cols.Length; i++)
                {
                    ...
                    for (int j = 0; j < rows.Length; j++)
                    {
                        ...
                        if (j > 0) lst.Add(new DvSplitter(rtTop, DvSplitterType.VERTICAL, i, j - 1));
                        if (i > 0) lst.Add(new DvSplitter(rtLeft, DvSplitterType.HORIZON, i - 1, j));
                        y += h;
                    }
                    x += w;
                }
    ...
    public class DvSplitter
    {
        ...
        public DvSplitter(Rectangle Bounds, DvSplitterType SplitterType, int ColIndex, int RowIndex)
        {
